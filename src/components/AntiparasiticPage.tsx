...
const AntiparasiticPage = () => {
  const navigate = useNavigate();
  const { selectedPet } = usePet();
  const [events, setEvents] = useState<CalendarEvent[]>([]);
  const [antiparasiticEvents, setAntiparasiticEvents] = useState<CalendarEvent[]>([]);
  ...
  // Filtrar eventos do tipo 'vermifugo' para o pet selecionado
  useEffect(() => {
    const filteredEvents = events.filter(
      (event) => event.type === "vermifugo" && event.petId === selectedPet.id
    );

    // Ordenar os eventos por data, do mais recente para o mais antigo
    const sortedAntiparasiticEvents = [...filteredEvents].sort(
      (a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()
    );

    setAntiparasiticEvents(sortedAntiparasiticEvents);
  }, [events, selectedPet]);
  ...
  const handleAddEvent = (newEvent: CalendarEvent) => {
    // Garantir que o tipo do evento seja 'vermifugo'
    const antiparasiticEvent = { ...newEvent, type: "vermifugo" as const };

    const eventExists = events.some(event => event.id === antiparasiticEvent.id);

    if (eventExists) {
      setEvents(prev =>
        prev.map(event =>
          event.id === antiparasiticEvent.id ? antiparasiticEvent : event
        )
      );
    } else {
      setEvents(prev => [...prev, antiparasiticEvent]);
    }
  ...
  const groupedEvents = groupEventsByMonthYear(antiparasiticEvents);
  const monthNames = [
    'Janeiro', 'Fevereiro', 'Março', 'Abril', 'Maio', 'Junho',
    'Julho', 'Agosto', 'Setembro', 'Outubro', 'Novembro', 'Dezembro'
  ];

  // Floating Action Button para adicionar vermífugo
  const fabButton = (
    <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
      ...
      <CalendarEventForm
        ...
        defaultEventType="vermifugo"
        formTitle="Adicionar Novo Vermífugo"
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-semibold">
                Histórico de Vermífugo de {selectedPet.name}
              </h2>
              <p className="text-sm text-gray-500">
                {antiparasiticEvents.length} vermífugos registrados
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg shadow p-4">
          {antiparasiticEvents.length > 0 ? (
                      ...
                      {monthEvents.map((event) => (
                        ...
                        >
                          <div className="flex justify-between items-start">
                            ...
                            <div className="flex flex-col space-y-1 items-end">                                  <Badge
                              variant="outline"
                              className="text-xs px-2 py-1 text-purple-600"
                            >
            ...
            <div className="text-center py-6 text-gray-500">
              ...
              <p className="text-sm">Nenhum vermífugo registrado para {selectedPet.name}</p>                  <Button
                variant="outline"
...
export default AntiparasiticPage;
